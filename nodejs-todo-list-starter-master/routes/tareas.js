...
async function modificarTarea(req, res, next) {
  ...
  if (!validAcceptHeader(acceptHeader)) {
    ...
    res.send({
      details:
        "Este endpoint solo permite 'application/json' media type, verifique sus cabecera `Accept` ",
      message: `No soporta :  ${acceptHeader}`
    });
    res.end();
    return;
  }
  ...
  try{
    ...
    res.statusCode = 400;
    res.send({
      message: `No se encuentra la tarea: ${idTarea}`,
      details: "Se espera un id valido"
    });
    res.end();
  }
}
...
/**
 * Elimina una tarea con identificador
 *
 * @param {object} req Petición HTTP
 * @param {object} res Respuesta HTTP
 */
