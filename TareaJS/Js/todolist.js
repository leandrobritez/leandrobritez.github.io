...
(() => {
    ...
    const getRequest = () => {
        ...
        xhr.open("GET", "https://task-backend-fpuna.herokuapp.com/tasks/", false);
        xhr.send(null);
        loadTasks(xhr.responseText);
    }
    ...
    const addTask = (e) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "https://task-backend-fpuna.herokuapp.com/tasks", false);
    ...
    const addOnChangeEvent = (task) => {
        ...
        checkBox.onchange = (e) => {
            ...
            let currentTask = new Task(currentDOMTask.querySelector("label").textContent);
            ...
            if(state){
                ...
                currentTask.status = TASK_STATUS.PENDING;
            }
            putRequest(currentTask);
    ...
    const putRequest = (task) => {
        const xhr = new XMLHttpRequest();
        let taskObj = new Task(task.description);
        taskObj.status = task.status;
        console.log(taskObj);

        xhr.open("PUT", "https://task-backend-fpuna.herokuapp.com/tasks/" + task.id, false);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.setRequestHeader('Accept', 'application/json');
    ...
    const removeTaskFromList = (id) => {
        ...
        let liEliminar = document.getElementById("task-" +id);
        liEliminar.remove();
    };
    ...
    const removeTask = (e) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open("DELETE", "https://task-backend-fpuna.herokuapp.com/tasks/" + id, false);
        xhr.send(null);
